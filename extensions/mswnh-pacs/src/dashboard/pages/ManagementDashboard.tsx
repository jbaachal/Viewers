...
export function ManagementDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : 'Management reports could not be loaded.'
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Operational performance and SLA analytics · Africa/Kampala time
          </p>
        </div>
      </div>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-8">
            <ManagementMetricCard
              ...
              detail="Received in selected period"
              onClick={() => openWorklist()}
            />
            <ManagementMetricCard
              label="Active backlog"
              value={report.summary.activeBacklog.toLocaleString()}
              detail="Received, assigned or in review"
              tone={report.summary.activeBacklog ? 'warning' : 'healthy'}
              onClick={() => openWorklist({ status: 'unread' })}
            />
            <ManagementMetricCard
              label="Emergency / urgent"
              value={report.summary.emergencyAndUrgent.toLocaleString()}
              detail="High-priority workload"
              tone={report.summary.emergencyAndUrgent ? 'critical' : 'healthy'}
            />
            <ManagementMetricCard
              label="Average TAT"
              value={minutes(report.summary.averageReportTurnaroundMinutes)}
              detail="Receipt to signed report"
            />
            <ManagementMetricCard
              label="Within SLA"
              value={percent(report.summary.reportedWithinSlaPercent)}
              detail="Signed by deadline"
            ...
            <ManagementMetricCard
              ...
              tone={report.summary.overdue ? 'critical' : 'healthy'}
              onClick={() => openWorklist({ overdue: 'true' })}
            />
            <ManagementMetricCard
              label="Exceptions"
          ...
          {report.summary.totalExaminations === 0 && (
            <div className="border-input/60 bg-card text-muted-foreground rounded-xl border p-8 text-center text-sm">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <ManagementBarChart
              ...
              description="Studies received per Kampala calendar day"
              data={report.dailyVolume}
            />
            <ManagementBarChart
              title="Examinations by modality"
              data={report.byModality}
              onSelect={item => openWorklist({ modality: item.label.toLocaleLowerCase() })}
            />
            <ManagementBarChart
              title="Workflow status"
              data={report.workflowStatuses}
              onSelect={item =>
                openWorklist({
                  status: item.label === 'InReview' ? 'in_review' : item.label.toLocaleLowerCase(),
                })
              }
            />
            <ManagementBarChart
              title="Clinical priority"
              data={report.priorities}
              onSelect={item => openWorklist({ priority: item.label.toLocaleLowerCase() })}
            />
            <ManagementBarChart
              title="Average turnaround by report date"
              description="Minutes from archive receipt to signed report"
              data={report.turnaroundTrend}
              suffix=" min"
            />
            <ManagementBarChart
              title="SLA compliance by priority"
              description="Percentage of signed reports completed within SLA"
              data={report.slaByPriority}
              suffix="%"
            />
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <ManagementBarChart
              ...
              data={report.cancellationReasons}
            />
            <ManagementBarChart
              title="Incomplete examination reasons"
              data={report.incompleteReasons}
            />
          </div>

          <p className="text-muted-foreground text-right text-xs">
            Generated{' '}
            {new Date(report.generatedAt).toLocaleString('en-GB', { timeZone: 'Africa/Kampala' })}
          </p>
        </>
      ) : null}
    </div>
